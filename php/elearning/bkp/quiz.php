...
$rs=mysql_query("select pretest_time from usertest where test_id=$tid and login='$_SESSION[login]'",$cn) or die(mysql_error());
...
<meta http-equiv="refresh" content="<?php echo($timeout) ?>;url=<?php echo($url); ?>">
	...
	<h1 style="text-align: center;">Redirecting to <a href="<?php echo $url ?>">index.php</a> in 
	<form name="counter">
...
{	
		...
		else if($submit=='Get Result' && isset($ans))
		{
				...
				mysql_query("insert into result(login,test_id,test_date,score) values('$login',$tid,'".$timestamp."',$_SESSION[trueans])") or die(mysql_error());
				echo "<h1 align=center><a href=review.php?testid=".$_SESSION[tid]."&"."subid=".$_SESSION[sid]."&questype=".$_SESSION[qid]."&totalques=".$_SESSION[qn]."> Review Question</a> </h1>";
				unset($_SESSION[qn]);
				unset($_SESSION[sid]);
				unset($_SESSION[tid]);
